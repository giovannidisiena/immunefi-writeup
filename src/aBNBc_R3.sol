...
contract aBNBc_R3 is OwnableUpgradeable, ERC20Upgradeable {
    ...
    modifier onlyMinter() {
        require(msg.sender == _binancePool || msg.sender == _bondToken, "Minter: not allowed");
        _;
    }

    function initialize(address binancePool, address bondToken) public initializer {
        __Ownable_init();
    ...
    function mintApprovedTo(address account, address spender, uint256 amount) external onlyMinter {
        _mint(account, amount);
    ...
    function changeBinancePool(address binancePool) external onlyOwner {
        ...
        require(AddressUpgradeable.isContract(binancePool), "non-contract address");
        _binancePool = binancePool;
    ...
    function changeBondToken(address bondToken) external onlyOwner {
        ...
        require(AddressUpgradeable.isContract(bondToken), "non-contract address");
        _bondToken = bondToken;
